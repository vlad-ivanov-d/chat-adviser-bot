...
describe("SettingsModule (e2e)", () => {
  ...
  it("refreshes chats", async () => {
    ...
    const response = await request(TEST_TG_WEBHOOK_BASE_URL).post(TEST_TG_WEBHOOK_PATH).send(fixtures.cbRefreshWebhook);
  ...
  it("renders the second page of chats", async () => {
    ...
    const response = await request(TEST_TG_WEBHOOK_BASE_URL).post(TEST_TG_WEBHOOK_PATH).send(fixtures.cbChatsWebhook);
